# OpenCode Steering Rules - Spec-Driven Development

## Purpose
This steering file ensures all OpenCode interactions adhere strictly to project specifications in /spec/{feature_name}/. Reference specs in every response. Apply these rules in all workflows and development phases.
...
- Leverage multi-session support for parallel development tasks
- Use LSP-enabled environment for accurate code intelligence
- Configure preferred LLM providers (Claude, OpenAI, etc.) for optimal performance
- Take advantage of native TUI for responsive terminal-based development
- Use session sharing links for collaboration and debugging
- Apply steering rules consistently across all sessions
- Focus on terminal-based workflows optimized for OpenCode's interface

This file steers all OpenCode interactions toward iterative, spec-driven development.